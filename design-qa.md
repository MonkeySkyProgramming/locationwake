...
- 参照画像の副操作「まず使ってみる」は、ユーザー指定により削除し、最初の操作を「設定を始める」だけにした。
...
- Interaction: 「設定を始める」から説明付きの位置情報ステップへ遷移し、遷移後は先頭へスクロールして見出しへフォーカスする。
